...
function reachesTargetViaGoto(start, target, seen = new Set()) {
	...
	// Also follow empty-stat conditional blocks (short-circuit pass-through for and/or)
	if (block.type === "conditional" && !block.stats?.filter(Boolean).length) {
		return reachesTargetViaGoto(block.trueNext, target, seen) ||
		       reachesTargetViaGoto(block.falseNext, target, seen);
	}
	return false;
}
...
function reconstruct(block, visited, stopVal, isInLoop, previous, loopExitNext = null) {
	...
	if (block.type === "goto") {
		...
		if (loopExitNext !== null && block.next === loopExitNext)
			return stats.concat({ type: "BreakStatement" });
		return stats.concat(
				reconstruct(nextBlock, nextVisited, stopVal, isInLoop, currId, loopExitNext),
	...
	if (block.type === "conditional") {
			...
			if (LastStats?.length) {
				...
				if (endSearch?.right && start && step) {
					...
					return [Statement].concat(
						reconstruct(findBlock(falseNext), nextVisited, stopVal, isInLoop, currId, loopExitNext),
					);
				}
			}
		}
		...
		if (trueLoops || (falseLoops && !is(getVar(stats, 0, true), condition))) {
			...
			else body = reconstruct(bodyBlock, nextVisited, null, true, currId, exitNext);
						...
						*/
						...
						if (forstat.iterators.length)
							return [forstat].concat(
								reconstruct(exitBlock, nextVisited, stopVal, isInLoop, currId, loopExitNext),
							);
					}
				}
			}
		...
		if (
			...
			if (andRhs && !is(andRhs, last)) {
				...
				return stats.concat(
					reconstruct(falseBlock, nextVisited, stopVal, isInLoop, currId, loopExitNext),
				);
			}
			...
			if (orRhs && !is(orRhs, last)) {
				...
				return stats.concat(
					reconstruct(trueBlock, nextVisited, stopVal, isInLoop, currId, loopExitNext),
				);
			} else if (join) {
				if (join != falseNext) {
		...
		if (join && join != falseNext) {
			const test = (falseBlock, a) => {
				...
				if (ig && isBetween(join, ig.range.min, ig.range.max)) {
					...
					return stats.concat(
						[ifStmt],
						reconstruct(ig, nextVisited, stopVal, isInLoop, currId, loopExitNext),
					);
				} else {
